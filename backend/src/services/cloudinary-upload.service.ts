...
export class CloudinaryUploadService implements StorageProvider {
    private async uploadStream(buffer: Buffer, publicId: string, width: number): Promise<string> {
        return new Promise((resolve, reject) => {
            ...
            (error, result) => {
                ...
                if(!result) return reject(new Error("Cloudinary Error ao processar imagem"))
    ...
    async uploadCover(file: Express.Multer.File): Promise<{ fullUrl: string; thumbUrl: string; }> {
